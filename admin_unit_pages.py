...
def listing(con, session, query, csrf, flash=""):
    ...
    def listing_url(target_page):
        values = {"q": search, "level": level, "active": status,
    ...
    <div class="card"><div class="catalog-list-heading"><div><strong>Danh sách đơn vị</strong><span>Lọc được {total_rows} bản ghi</span></div><form method="get" class="catalog-list-controls"><input type="hidden" name="q" value="{esc(search)}"><input type="hidden" name="level" value="{esc(level)}"><input type="hidden" name="active" value="{esc(status)}"><label for="catalog-page-size">Hiển thị</label><select id="catalog-page-size" name="page_size" onchange="this.form.submit()" aria-label="Số dòng mỗi trang">{page_size_options}</select></form></div><div class="table-wrap"><table class="summary-table catalog-table"><thead><tr><th>Mã đơn vị</th><th>Tên đơn vị</th><th>Cấp hành chính</th><th>Mã cấp trên</th><th>Trạng thái</th><th>Số tài khoản liên kết</th><th>Thao tác</th></tr></thead>
    <tbody>{trs or '<tr><td colspan="7" class="empty">Không tìm thấy đơn vị phù hợp.</td></tr>'}</tbody></table></div>{pagination}</div></div>'''
