...
DB_PATH = os.path.join(BASE_DIR, "salt_management.db")
ASSET_FILES = {
...
def base_page(title, body, session=None):
    top = ""
    if session:
        current = {
            "Tổng quan": "/dashboard",
        ...
        nav_items = [
            ("/dashboard", "dashboard", "Bảng giám sát"),
            ("/records", "table", "Dữ liệu báo cáo"),
        ...
        nav_items.append(("/change-password", "key", "Đổi mật khẩu"))
        nav = []
        for href, symbol, label in nav_items:
        ...
        <header class="site-header">
          ...
          <a class="brand-link" href="/dashboard">
            ...
            <div class="brand-copy"><div class="brand-agency">{display_label}</div><div class="brand-subtitle">QUẢN LÝ SỐ LIỆU SẢN XUẤT MUỐI</div></div>
          </a>
        ...
        <aside class="sidebar" id="site-sidebar" aria-label="Menu chính">
          <form class="sidebar-search" action="/records" method="get" role="search"><button type="submit" aria-label="Tìm báo cáo">{icon('search')}</button><input name="q" type="search" placeholder="Tìm báo cáo..." aria-label="Tìm theo đơn vị hoặc ghi chú"></form>
          <div class="sidebar-section">QUẢN LÝ SỐ LIỆU</div>
          <nav class="sidebar-nav" aria-label="Chức năng">{''.join(nav)}<a class="sidebar-link" href="/logout" title="Đăng xuất">{icon('logout')}<span class="sidebar-label">Đăng xuất</span></a></nav>
    ...
    return f"""<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{esc(title)} · Quản lý muối</title><link rel="icon" href="/assets/quoc-huy.png" type="image/png"><link rel="stylesheet" href="/assets/app.css?v=20260908-red"><script src="/assets/app.js?v=20260908-red" defer></script></head><body>{top}{body}</body></html>"""
...
def login_page(message=""):
    ...
    <div class="login-wrap"><div class="login">
      ...
      <h1>Hệ thống tổng hợp dữ liệu diêm nghiệp</h1>
...
def records_page(session, query):
    ...
    <div class="container">
      {take_flash(session)}
      <div class="page-head"><div><h1>Dữ liệu báo cáo</h1><div class="subtitle">Tra cứu, lọc và xuất dữ liệu theo kỳ báo cáo/đơn vị/trạng thái.</div></div><div class="actions"><a class="btn primary" href="/records/new">+ Nhập số liệu</a><a class="btn ok" href="/export.xlsx?{query_string}">Xuất Excel</a></div></div>
    ...
    <div class="container">
      <div class="page-head">
        ...
        <div>
          ...
          <div class="subtitle">
            Các cột “Cộng” và “Năng suất bình quân” được hệ thống tự tính.
          </div>
        </div>

        <div class="actions">

          <a class="btn ok" href="/import-excel">
            {icon('download')} Nhập dữ liệu từ Excel
          </a>
    ...
    def do_POST(self):
        ...
        if (
            ...
            if action == "delete":
                ...
                if record_count > 0 or log_count > 0:
        ...
        if len(parts)>=3 and parts[0]=="records" and parts[1].isdigit():
            ...
            if action=="approve":
                if session["role"]!=ROLE_ADMIN or r["status"]!=STATUS_SUBMITTED: con.close(); self.send_html(base_page("Lỗi","<div class='container'><div class='notice err'>Không thể duyệt báo cáo này.</div></div>",session),400); return
                con.execute("UPDATE records SET status='approved',approved_at=?,reviewer_note='',updated_at=? WHERE id=?",(now_text(),now_text(),rid)); add_audit(con,rid,session["user_id"],"Phê duyệt","Chấp nhận số liệu"); con.commit(); con.close(); set_flash(session,"ok","Đã phê duyệt số liệu."); self.redirect(f"/records/{rid}"); return
            if action=="return":
...
def main():
    ...
    print("="*72)
    print("HỆ THỐNG QUẢN LÝ SỐ LIỆU SẢN XUẤT MUỐI")
    print(f"Đang chạy tại: http://127.0.0.1:{PORT}")
    print(f"Trong mạng LAN: http://<IP-máy-chủ>:{PORT}")
