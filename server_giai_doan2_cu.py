...
def _temporary_unit_code(unit_name):
    """Mã nội bộ tạm thời, ổn định, tối đa 10 ký tự. Sẽ thay bằng mã hành chính chính thức sau."""
    digest = hashlib.sha1(unit_name.casefold().encode("utf-8")).hexdigest()[:7].upper()
    return "TMP" + digest


def ensure_standard_unit(con, unit_name):
    """Tìm mã đơn vị chuẩn; nếu chưa có thì tạo mã nội bộ TMPxxxxxxx để không mất dữ liệu."""
    row = con.execute(
